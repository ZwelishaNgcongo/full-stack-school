...
// Fetch classes for a specific grade with assignment counts
async function getClassesForGrade(gradeId: number) {
  ...
  return classes.map(cls => ({
    ...
    assignmentCount: cls.lessons.reduce((acc, lesson) => acc + lesson.assignments.length, 0)
  }));
}
...
const AssignmentViewPage = async ({ searchParams }: AssignmentViewPageProps) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 p-6">
      <div className="max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Select a grade and class to view assignments
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {grades.map((grade) => {
                ...
                return (
                  <Link
                    key={grade.id}
                    href={`/list/assignments/view?gradeId=${grade.id}`}
          ...
          {/* Class Selection */}
          <div className={`bg-white rounded-2xl shadow-lg border-2 border-blue-100 p-6 transition-all duration-300 ${
            ...
            {!selectedGradeId ? (
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {classes.map((cls) => (
                  <Link
                    key={cls.id}
                    href={`/list/assignments/view/${cls.id}`}
                    className="block w-full p-4 rounded-xl text-left transition-all duration-300 bg-gray-50 hover:bg-gradient-to-r hover:from-blue-500 hover:to-cyan-500 hover:text-white hover:shadow-lg hover:scale-105 text-gray-700"
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-semibold">{cls.name}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-xs px-2 py-1 bg-white/20 rounded-full">
                          {cls.assignmentCount} assignment{cls.assignmentCount !== 1 ? "s" : ""}
                        </span>
        ...
        {!selectedGradeId && (
          <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-500">Choose a grade and class to view their assignments</p>
          </div>
        )}
      </div>
    </div>
